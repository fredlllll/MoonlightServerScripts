...
from Settings.Settings import SESSION_COOKIE_NAME
...
class Session(Model):
    ...
    def set_cookie(self, handler):
        handler.set_secure_cookie(SESSION_COOKIE_NAME, self.model_id.encode())

    def clear_cookie(self, handler):
        handler.clear_cookie(SESSION_COOKIE_NAME)

    @classmethod
    def get_from_cookie(cls, handler):
        session_id_bytes = handler.get_secure_cookie(SESSION_COOKIE_NAME)
