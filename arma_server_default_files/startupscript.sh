#!/bin/bash
cd "/home/moonlight/.steam/SteamApps/common/Arma 3 Server"
./arma3server_x64 -cfg=basic.cfg -config=server.cfg -name=server -loadMissionToMemory -mod="\
mods/@Breaching Charge;\
mods/@CBA_A3;\
mods/@CUP Units;\
mods/@CUP Weapons;\
mods/@DS Houses;\
mods/@ACE Compat - RHS United States Armed Forces;\
mods/@Tactical Position Ready;\
mods/@GRAD Trenches;\
mods/@BackpackOnChest;\
mods/@ACE 3 Extension (Animations and Actions);\
mods/@Kunduz, Afghanistan Revival Project;\
mods/@ace;\
mods/@UnderSiege Patches & Insignias;\
mods/@CUP ACE3 Compatibility Addon - Terrains;\
mods/@GGE- Weapon Canting;\
mods/@PLP Containers;\
mods/@Zeus Enhanced - ACE3 Compatibility;\
mods/@CLA CLAFGHAN;\
mods/@MLO All-In-One Collection;\
mods/@Loadout Transfer 2;\
mods/@GRAD SlingHelmet;\
mods/@GGE- Captive Animations;\
mods/@Alpha Group Equipment;\
mods/@Task Force Arrowhead Radio (BETA!!!);\
mods/@USP Gear & Uniforms;\
mods/@GGE- NVG Animation;\
mods/@CUP ACE3 Compatibility Addon - Vehicles;\
mods/@PLP Urban Packs- Barriers and Fences;\
mods/@Pulau;\
mods/@USAF MOD Fighters Pack;\
mods/@USAF MOD Utility pack;\
mods/@Kujari;\
mods/@Blastcore Edited (standalone version);\
mods/@CUP Terrains - Core;\
mods/@Advanced Towing;\
mods/@RHSUSAF;\
mods/@Splendid Smoke;\
mods/@Island Panthera;\
mods/@Virolahti - Valtatie 7;\
mods/@USAF MOD Main Pack;\
mods/@USSOC_LITE_mas;\
mods/@ACEX;\
mods/@CUP Vehicles;\
mods/@Zeus Enhanced;\
mods/@DUI - Squad Radar;\
mods/@LAMBS_Danger.fsm;\
mods/@Enhanced Movement;\
mods/@6 x 6 All Terrain Vehicle;\
mods/@CH View Distance;\
mods/@CUP ACE3 Compatibility Addon - Weapons;\
mods/@Advanced Urban Rappelling;\
mods/@GGE- Core;\
mods/@Tactical Weapon Swap;\
"